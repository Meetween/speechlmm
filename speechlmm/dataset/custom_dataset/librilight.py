...
class LibriLightDataset(CustomAudioDataset, ConditioningMixin):
    name = "LibriLight"
    codename = "librilight"
    ...
    optional_filters = [CTC_ALIGNER_FILTER]
    ...
    def _build_speakers_map(self):
        ...
        def add_to_speakers_map(idx, example):
            speakers_map[example["speaker_id"]].append(idx)
    ...
    def _get_dataset_path(
        ...
        stem = f"{source_language}/{partition_name}"
        filename = f"{stem}.parquet"  # language_code/partition_name.parquet
    ...
    def get_conditioning_for_example(
        ...
        while not found_valid_conditioning:
            ...
            if candidate_example["text"] != example["transcription"]:
