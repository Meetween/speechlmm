...
from speechlmm.eval.utils.tts.zh_num2words import TextNorm as zh_num2words
    ...
    "cs": [
        ...
        for x in [
            ...
            (
                "p",
                "pan",
            ),  # Could also map to pani for woman but no easy way to do it
...
def _expand_ordinal(m, lang="en"):
    return num2words(
        int(m.group(1)), ordinal=True, lang=lang if lang != "cs" else "cz"
    )
...
def expand_numbers_multilingual(text, lang="en"):
    if lang == "zh":
        ...
        try:
            text = re.sub(
                _currency_re["GBP"],
                lambda m: _expand_currency(m, lang, "GBP"),
                text,
            )
            text = re.sub(
                _currency_re["USD"],
                lambda m: _expand_currency(m, lang, "USD"),
                text,
            )
            text = re.sub(
                _currency_re["EUR"],
                lambda m: _expand_currency(m, lang, "EUR"),
                text,
            )
        except:
            pass
        if lang != "tr":
            text = re.sub(
                _decimal_number_re,
                lambda m: _expand_decimal_point(m, lang),
                text,
            )
        text = re.sub(
            _ordinal_re[lang], lambda m: _expand_ordinal(m, lang), text
        )
...
class WhisperSTT(object):
    def __init__(self, model_name="large-v3") -> None:
    ...
    def transcribe_audio(self, audio, language="english"):
        result = self.pipe(audio, generate_kwargs={"language": language})
        transcription = result["text"].strip()
        # convert number to words
        transcription = custom_expand_numbers_multilingual(
...
class FasterWhisperSTT(object):
    def __init__(self, model_name="large-v3", use_cuda=True) -> None:
...
class Wav2VecSTT(object):
    ...
    def transcribe_audio(self, audio, language=None):
        ...
        input_values = self.tokenizer(
            input_audio, return_tensors="pt", padding="longest"
        ).input_values
...
class MMSWav2VecSTT(object):
    def __init__(
        self, model_name="facebook/mms-1b-all", use_cuda=False
    ) -> None:
        self.pipe = pipeline(
            model=model_name, device="cuda:0" if use_cuda else "cpu"
        )
