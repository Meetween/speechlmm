...
class CTC(torch.nn.Module):
    ...
    def __init__(
        self, odim, eprojs, dropout_rate, ctc_type="builtin", reduce=True
    ):
        super().__init__()
        ...
        if self.ctc_type == "builtin":
            ...
            from speechlmm.model.encoders.video_encoders.auto_avsr_modules.espnet.nets.pytorch_backend.gtn_ctc import (
                GTNCTCLossFunction,
            )
    ...
    def forced_align(self, h, y, blank_id=0):
        ...
        logdelta = (
            np.zeros((lpz.size(0), len(y_int))) - 100000000000.0
        )  # log of zero
        state_path = (
    ...
    def forced_align_batch(self, hs_pad, ys_pad, ilens, blank_id=0):
        ...
        def interpolate_blank(label, olens_int):
            ...
            label_out = np.full(
                (label.shape[0], lab_len), blank_id, dtype=np.int64
            )
            label_out[:, 1::2] = label
