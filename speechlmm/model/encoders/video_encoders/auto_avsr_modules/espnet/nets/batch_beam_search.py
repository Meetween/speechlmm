...
from speechlmm.model.encoders.video_encoders.auto_avsr_modules.espnet.nets.beam_search import (
    BeamSearch,
    Hypothesis,
)
...
class BatchBeamSearch(BeamSearch):
    ...
    def batchfy(self, hyps: List[Hypothesis]) -> BatchHypothesis:
        ...
        return BatchHypothesis(
            ...
            scores={
                k: torch.tensor(
                    [h.scores[k] for h in hyps], device=yseq.device
                )
    ...
    def merge_states(
        self, states: Any, part_states: Any, part_idx: int
    ) -> Any:
    ...
    def search(
        self, running_hyps: BatchHypothesis, x: torch.Tensor
    ) -> BatchHypothesis:
        ...
        scores, states = self.score_full(
            running_hyps, x.expand(n_batch, *x.shape)
        )
        ...
        if self.do_pre_beam:
            ...
            part_ids = torch.topk(pre_beam_scores, self.pre_beam_size, dim=-1)[
                1
            ]
