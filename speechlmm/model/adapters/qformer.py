...
class BertSelfAttention(nn.Module):
    def __init__(self, config, is_cross_attention):
        ...
        if is_cross_attention:
            self.key = nn.Linear(
                config.cross_attention_hidden_size, self.all_head_size
            )
...
class BertLayer(nn.Module):
    def __init__(self, config, layer_num):
        ...
        if (
            ...
            and layer_num % self.config.cross_attention_every_n_layers == 0
        ):
...
class BertModel(BertPreTrainedModel):
    ...
    _keys_to_ignore_on_load_missing = [
        ...
        "encoder.layer.[0-9]+.intermediate",
        "encoder.layer.[0-9]+.output",
    ]
    ...
    def forward(
        ...
        past_key_values_length = (
            past_key_values[0][0].shape[2] - self.config.num_queries
        ...
        if encoder_hidden_states is not None:
            ...
            if type(encoder_attention_mask) == list:
                ...
                # print("encoder_extended_attention_mask", encoder_extended_attention_mask.size())
        else:
