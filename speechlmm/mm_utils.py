...
def process_audios(audios_srs, audio_processor, model_cfg):
    ...
    for audio, sr in audios_srs:
        # check if audio is multidimensional
        if audio.dim() > 1 and audio.shape[0] > 1:
            audio = audio.mean(dim=0, keepdim=True)

        if target_sampling_rate is not None:
            audio = T.Resample(sr, target_sampling_rate)(audio)
            sr = target_sampling_rate
        new_audios_srs.append((audio, sr))
    return new_audios_srs


def monify_and_resample_audio(
    audio: torch.Tensor,
    orig_sr: int,
    target_sr: Optional[int] = None,
) -> Tuple[torch.Tensor, int]:
    target_sr = target_sr or orig_sr
    audio = T.Resample(orig_sr, target_sr)(audio)
    return audio.mean(dim=0, keepdim=True), target_sr
...
def tokenizer_mm_token(
    ...
    logging.warning("tokenizer_mm_token is deprecated")
    # Find positions of both audio and image tokens in the prompt
    audio_matches = [
        (m.start(), m.end(), audio_token_index)
        for m in re.finditer(DEFAULT_AUDIO_TOKEN, prompt)
    ]
    image_matches = [
        (m.start(), m.end(), image_token_index)
        for m in re.finditer(DEFAULT_IMAGE_TOKEN, prompt)
    ]

    video_matches = [
        (m.start(), m.end(), video_token_index)
        for m in re.finditer(DEFAULT_VIDEO_TOKEN, prompt)
    ]

    # Combine and sort matches by start position
    matches = sorted(
        audio_matches + image_matches + video_matches, key=lambda x: x[0]
    )

    # Initialize input_ids list with BOS token if present
    input_ids = []

    # Tokenize and split prompt by multimodal tokens
    prev_end = 0
...
def get_model_type_from_path(model_path: Union[str, Path]):
    model_path = Path(model_path)
    config_path = model_path / "config.json"
    if not config_path.exists():
        raise ValueError(f"Could not get model type from {config_path}")
...
class KeywordsStoppingCriteria(StoppingCriteria):
    ...
    def call_for_batch(
        ...
        offset = min(
            output_ids.shape[1] - self.start_len, self.max_keyword_len
        )
        self.keyword_ids = [
